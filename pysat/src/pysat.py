...
class Solver():
            ...
            for j in range(0 if p is None else 1, len(c)):
                ...
                if (self._seen[v] == 0) and (self._level[v] > 0):
                    ...
                    if self._level[v] == self._decisionLevel():
                        ...
                        learnt.append(
                            q)  # q is a literal in the learnt clause number of literals from the last decision level in the conflict analysis
                        if self._level[v] > backtrackLevel:  # Updates the backtrackLevel
                            backtrackLevel = self._level[v]  # It is the max of seen levels
                            maxbl = len(
                                learnt) - 1  # We need to keep its indice: we'll rearrange the clause before backtracking
    ...
    def _reportSearch(self):
        print(
            "c {cfl:d} conflicts, {prop:d} propagations, {rest:d} restarts, {una:d}/{unalearnts:d} unaries, {depth:d} decisions depth, {propdepth:d} propagation depth, {res:d} resolutions".format(
                cfl=self._conflicts,
